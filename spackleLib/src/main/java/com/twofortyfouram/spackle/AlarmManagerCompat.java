...
public final class AlarmManagerCompat {
    ...
    public AlarmToken setExact(@NonNull final Context context, @AlarmType final int type,
        ...
        assertInRangeInclusive(type, AlarmManager.RTC_WAKEUP,
                AlarmManager.ELAPSED_REALTIME, "type"); //$NON-NLS-1$
        assertNotNull(pendingIntent, "pendingIntent"); //$NON-NLS-1$

        if (AndroidSdkVersion.isAtLeastSdk(Build.VERSION_CODES.M)) {
            return setExactMarshmallow(type, triggerAtMillis, pendingIntent, currentWalltimeMillis,
                    currentElapsedRealtimeMillis);
        } else if (AndroidSdkVersion.isAtLeastSdk(Build.VERSION_CODES.LOLLIPOP)) {
    ...
    public void cancel(@NonNull final AlarmToken alarmToken) {
        assertNotNull(alarmToken, "alarmToken"); //$NON-NLS-1$
    ...
    private static final class AlarmRunnable implements Runnable {
        ...
        public AlarmRunnable(@NonNull final PendingIntent pendingIntent,
            ...
            assertNotNull(pendingIntent, "pendingIntent"); //$NON-NLS-1$
            assertNotNull(partialWakeLock, "partialWakeLock"); //$NON-NLS-1$
